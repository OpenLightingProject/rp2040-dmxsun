...
if [ ! -f makefsdata ]; then
    ...
    if [ -z "$CI" ]; then
        ...
        gcc -o makefsdata -DMAKEFS_SUPPORT_DEFLATE=1 -DMAKEFS_SUPPORT_DEFLATE_ZOPFLI=1 -Ilib/lwip/src/include -I/usr/include/zopfli -Isrc lib/lwip/src/apps/http/makefsdata/makefsdata.c -lz -lzopfli
    fi
fi
